  ...
  describe("tests where backend is working normally", () => {
    ...
    beforeEach(() => {
      ...
      axiosMock
        ...
        .reply(200, systemInfoFixtures.showingNeither);
      axiosMock
        .onGet("/api/UCSBOrganization", { params: { orgCode: "20221" } })
        .reply(200, {
          orgCode: "20221",
          orgTranslation: "Pi Day",
          orgTranslationShort: "Pi",
          inactive: false,
        });
      axiosMock.onPut("/api/UCSBOrganization").reply(200, {
        orgCode: "20221",
        orgTranslation: "Christmas Morning",
        orgTranslationShort: "Xmas",
        inactive: true,
      });
    });
    ...
    test("Is populated with the data provided", async () => {
      ...
      expect(orgCode).toHaveValue("20221");
      expect(orgTranslation).toHaveValue("Pi Day");
      expect(orgTranslationShort).toHaveValue("Pi");
    ...
    test("Changes when you click Update", async () => {
      ...
      fireEvent.change(orgCode, { target: { value: "20221" } });
      fireEvent.change(orgTranslation, {
        target: { value: "Christmas Morning" },
      });
      fireEvent.change(orgTranslationShort, { target: { value: "Xmas" } });
      ...
      expect(mockToast).toBeCalledWith(
        "UCSBOrganization Updated - Organization Code: 20221"
      );
      expect(mockNavigate).toBeCalledWith({ to: "/ucsborganization" });

      expect(axiosMock.history.put.length).toBe(1); // times called
      expect(axiosMock.history.put[0].params).toEqual({ orgCode: "20221" });
